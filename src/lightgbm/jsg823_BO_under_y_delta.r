...
PARAM$experimento <- "HT8230_best_public_score"
...
dataset[, (columnas_a_calcular) := lapply(.SD, function(x) rank(-x)),
...
dataset[, paste(columnas_a_calcular, "delta", sep = "_") := .SD - lag_1, by = numero_de_cliente, 
        .SDcols = columnas_a_calcular]

dataset[, paste(columnas_a_calcular, "delta_1", sep = "_") := lag_1 - lag_2, by = numero_de_cliente, 
        .SDcols = columnas_a_calcular]
